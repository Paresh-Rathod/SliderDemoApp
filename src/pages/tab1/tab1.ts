...
/* gotta check if they are all actually needed at some point */
...
export class Tab1Page {
 ...
 // constructor(public navCtrl: NavController, public navParams: NavParams) {
  ...
  this.itemsRef.query.on("child_added", function(snapshot){ 
    ...
    var userPlace = snapshot.child("myObj/Name").val();
    var userZone = snapshot.child("myObj/Zone").val();
  ...
  currentTime(adjTime: number) { //get current time
    ...
    var myDate = new Date(timeNow + (1000 * 60 * 60 * adjTime));
  ...
  */
  ...
  {
    var myTime = this.currentTime(this.myZone + this.selectedZone);
